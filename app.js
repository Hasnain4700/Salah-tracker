...
function showSection(section) {
  ...
  navBtns[["home","donate","quran","tracker"].indexOf(section)].classList.add('active');
}
...
async function requestNotificationPermission() {
  ...
  try {
    ...
    // You can get your VAPID key from Firebase Console > Project Settings > Cloud Messaging
    const token = await getToken(messaging, { vapidKey: 'BBeVQ0f8nC--oymwOnsGfla9p5AB5h37TEPpf1EMY0QTz4pbdPjlmqn-8Rkjw8sAE71ksSnkqcvRpA7M0_64FBE' });
    console.log('FCM Token:', token);
    // Optionally, save this token to your database for targeted notifications
    // Subscribe to general topic (for auto notifications)
    // (Topic subscription for web is not directly supported, but you can manage tokens server-side)
  } catch (err) {
...
if (messaging) {
  onMessage(messaging, (payload) => {
    ...
    // Optionally, show a custom notification or update UI
  });
}
